...
def run_gemini(client, content: str, model_name: str = "gemini-2.0-flash", max_tokens: int = 0):
    max_retries = 3
    retry_delay = 35  # Based on the retry delay in the error message
    
    for attempt in range(max_retries):
        try:
            # Create content in the new format
            contents = [
                types.Content(
                    role="user",
                    parts=[
