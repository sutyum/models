...
def analyze_aggr_acc(
    ann_file, in_file, out_file, model_name, metric_key, encoder=None, rag=False
):
    ...
    for sample_id in sample_ids:
        ...
        for i, qa in tqdm(enumerate(output["qa"])):
            ...
            if metric_key in qa:
                ...
                qa["evidence"] = [
                    q.replace("(", "").replace(")", "") for q in qa["evidence"]
                ]
                        ...
                        try:
                            ...
                            farthest_dialog = min(
                                [
                                    ...
                                    for e in qa["evidence"]
                                    if e != ""
                                    and int(e.split(":")[0][1:]) == farthest_session
                                ]
                            )

                            farthest_length = id2length[
                                "D" + str(farthest_session) + ":" + str(farthest_dialog)
                            ]

                            memory_counts_og[qa["category"]][
                                math.ceil(farthest_length / 1000)
                            ] += 1
                            memory_counts[qa["category"]][
                                math.ceil(farthest_length / 1000)
                            ] += qa[metric_key]
                            ...
                            if qa["category"] == 1:
                                ...
                                latest_dialog = max(
                                    [
                                        ...
                                        for e in qa["evidence"]
                                        if e != ""
                                        and int(e.split(":")[0][1:]) == latest_session
                                    ]
                                )

                                latest_length = id2length[
                                    "D" + str(latest_session) + ":" + str(latest_dialog)
                                ]
                                context_length = latest_length - farthest_length
                                context_len_og[math.ceil(context_length / 1000)] += 1
                                context_len_counts[
                                    math.ceil(context_length / 1000)
                                ] += qa[metric_key]
                        except:
                            continue
            else:
    ...
    if rag:
        results_dict[model_name]["recall_by_category"] = {
            k: v / total_counts[k] for k, v in recall_by_category.items()
        }
        ...
        print(
            ...
            sum(list(recall_by_category.values())) / sum(list(total_counts.values())),
        )
    else:
